...
class Subscribers_controller extends BaseController
{
    ...
    public function subscribersListView(){
        ...
        $this->global['pageTitle'] = 'Radius : Subscribers Listing';
        $data['managers'] = $this->subscribers_model->getManagersList();
        $data['services'] = $this->subscribers_model->getResellerPackages();
        $this->loadViews("subscriber/subscriberslist", $this->global, $data, NULL);
    }

    public function subscriberAddNotInUse(){
        $this->global['pageTitle'] = 'Pace-Tel : User Listing';
        $this->loadViews("subscriber/subscriberadd", $this->global, NULL, NULL);
    }

    // Subscriber View
    public function subscriberView(){

        $perm_listusers = $this->session->userdata('perm_listusers');

        if($perm_listusers == 1){

            $this->global['pageTitle'] = 'RadPro : Subscriber';
            $this->loadViews("subscriber/subscriberslist", $this->global, NULL, NULL);

        }else{

            $this->global['pageTitle'] = 'Pace-Tel : Error';
            $data['errorTitle'] = "User not allowed to view this page.";
            $this->loadViews('error', $this->global, $data, NULL);
    ...
    function getSubscribersList()
    {
        ...
        //if($perm_listusers == 1 || $perm_listusers == null){
            ...
            $data = $this->subscribers_model->subscribersList($searchText, $searchText1, $type, $owner, $srvid, $length, $start, $orderColumnIndex, $orderDir);

            //echo(json_encode(array($data)));
            //exit;
            //log_message('info', 'subscribers-list: '.json_encode($data));
            ...
            foreach ($records as $row) {
                ...
                $nestedData['device_type'] = isset($row->device_type) ? $row->device_type : null;
                $nestedData['session_index'] = isset($row->session_index) ? $row->session_index : null;
                $output['data'][] = $nestedData;
            }

            //log_message('info', 'subscribers-list - Subscribers List Output: '.json_encode($output));
    ...
    function subscriberAdd()
    {
        ...
        $perm_createusers = $this->session->userdata('perm_createusers');

        if($perm_createusers == 1){
            ...
            $this->loadViews("subscriber/subscriberadd", $this->global, $data, NULL);
        }else{
    ...
    function saveSubscriber(){
        ...
        if($perm_createusers == 1){
            ...
            $managername = $this->input->post('manager');
            ...
            if(!empty($managername)){
                ...
                $userInfo = array('username'=>$user, 'password'=>MD5($password), 'groupid'=>1, 'enableuser'=> 1,
                            ...
                            'usemacauth'=>0, 'expiration'=>date('Y-m-d'), 'uptimelimit'=>0, 'srvid'=>0, 
                ...
                $logInfo = 'user: '.$user.'-owner: '.$managername.'-createdby: '.$this->session->userdata ( 'name' );
                log_message('info', 'subscribers-list - Created User '.$logInfo);
                ...
                if($this->users_model->checkUserExist($user) == FALSE)
                {
                    ...
                    if($result == True)
                    {
                        redirect('subscribers/subscribersListView');
                    }

                }else{
                    //redirect('Subscribers/subscriberAdd');
                }
            }
    ...
    function subscriberProfile($username = null){

        $data['userInfo'] = $this->users_model->getUserInfo($username);
        $data['docsInfo'] = $this->users_model->getUserDocsInfo($username);

        $data['radcheck'] = $this->users_model->getAttributes_radcheck($username, "Simultaneous-Use");
        ...
        $this->loadViews('subscriber/subscriberProfile', $this->global, $data, NULL);

    }

    // Subscriber Update
    function update_subscriberProfile()
    {

        $expiration = $this->security->xss_clean($this->input->post('expiration'));
        $expdate_time = $this->security->xss_clean($this->input->post('expiration_time'));
        $subscriberUsername = $this->security->xss_clean($this->input->post('username'));

        $expdate_obj = DateTime::createFromFormat('d F Y', $expiration);

        if ($expdate_obj) {
            $formatted_duedate = $expdate_obj->format('Y-m-d');
        } else {
            // Handle error if duedate format is incorrect
            echo json_encode(array('status' => 'error', 'message' => 'Invalid due date format. date:'.$this->input->post('expiration')));
            return;
        }

        $exp_datetime = $formatted_duedate . ' ' . date("H:i:s", strtotime($expdate_time));


        // Get form input data
        $data = array(
            'username' => $this->security->xss_clean($this->input->post('username')),
            'enableuser' => $this->security->xss_clean($this->input->post('enableuser')) ? 1 : 0,
            'verified' => $this->security->xss_clean($this->input->post('verified')) ? 1 : 0,
            //'password' => MD5($this->security->xss_clean($this->input->post('password1'))),
            'alertemail' => $this->security->xss_clean($this->input->post('alertemail')) ? 1 : 0,
            'alertsms' => $this->security->xss_clean($this->input->post('alertsms')) ? 1 : 0,
            'downlimit' => $this->security->xss_clean($this->input->post('tot_download')),
            'uplimit' => $this->security->xss_clean($this->input->post('tot_upload')),
            'comblimit' => $this->security->xss_clean($this->input->post('tot_combined')),
            'mac' => $this->security->xss_clean($this->input->post('mac')),
            'usemacauth' => $this->security->xss_clean($this->input->post('usemacauth')) ? 1 : 0,
            'ipmodecpe' => $this->security->xss_clean($this->input->post('ipmodecpe')),
            'staticipcpe' => $this->security->xss_clean($this->input->post('staticip')),
        ...
        $password_input = $this->security->xss_clean($this->input->post('password1'));
        ...
        $result = $this->users_model->update_subscriber($data);

        // Send the response back to AJAX call
        if ($result) {

            $rad_password = $this->security->xss_clean($this->input->post('password3'));

            if (!empty($rad_password)) {

                $radpassword = array('attribute'=>'Cleartext-Password', 'op'=>':=', 'value'=>$rad_password);
                //$radsimuse = array('attribute'=>'Simultaneous-Use', 'op'=>':=', 'value'=>'1');

                $radcheckResult = $this->users_model->editUserPasswordRadCheck($radpassword, $this->security->xss_clean($this->input->post('username')));
            ...
            // Handle the file uploads using the helper function
            $uploadedFiles['cnic_file1'] = $this->upload_file('cnic_file1', $subscriberUsername.'_cnic_file1_'.time());
            $uploadedFiles['cnic_file2'] = $this->upload_file('cnic_file2', $subscriberUsername.'_cnic_file2_'.time());
            $uploadedFiles['agreement_file1'] = $this->upload_file('agreement_file1', $subscriberUsername.'_agreement_file1_'.time());
            $uploadedFiles['agreement_file2'] = $this->upload_file('agreement_file2', $subscriberUsername.'_agreement_file2_'.time());


            $dataUserDocs = array(
                'username' => $this->security->xss_clean($this->input->post('username')),
                'cnicno' => $this->security->xss_clean($this->input->post('cnicno')),
                'cnic_file1' => isset($uploadedFiles['cnic_file1']) ? $uploadedFiles['cnic_file1'] : null,
                'cnic_file2' => isset($uploadedFiles['cnic_file2']) ? $uploadedFiles['cnic_file2'] : null,
                'agreement_file1' => isset($uploadedFiles['agreement_file1']) ? $uploadedFiles['agreement_file1'] : null,
                'agreement_file2' => isset($uploadedFiles['agreement_file2']) ? $uploadedFiles['agreement_file2'] : null,
                'discount' => $this->security->xss_clean($this->input->post('discount')),
                'adjamount' => $this->security->xss_clean($this->input->post('adjamount')),
                'payid' => $this->security->xss_clean($this->input->post('payid')),
                'payname' => $this->security->xss_clean($this->input->post('payname')),
                'inst_name' => $this->security->xss_clean($this->input->post('inst_name')),
                'inst_box' => $this->security->xss_clean($this->input->post('inst_box')),
                'inst_box_sn' => $this->security->xss_clean($this->input->post('inst_box_sn')),
                'inst_wifi' => $this->security->xss_clean($this->input->post('inst_wifi')),
                'inst_fiber' => $this->security->xss_clean($this->input->post('inst_fiber')),
                'inst_meter' => $this->security->xss_clean($this->input->post('inst_meter')),
                'inst_chrg' => $this->security->xss_clean($this->input->post('inst_chrg')),
                'inst_cost' => $this->security->xss_clean($this->input->post('inst_cost')),
                'inst_disc' => $this->security->xss_clean($this->input->post('inst_disc'))
            );

            // Update Users Documents
            $this->users_model->updateDocsInfo($dataUserDocs, $subscriberUsername);

            echo json_encode(['status' => 'success', 'message' => 'User details updated successfully.'.$result]);

        } else {
            echo json_encode(['status' => 'error', 'message' => 'Failed'.$result]);
        }
    }
    ...
    public function loadRechargeForm()
    {
        ...
        $perm_edituserspriv = $this->session->userdata('perm_edituserspriv');

        if($perm_edituserspriv == 1){
    ...
    private function upload_file($field_name, $file_name) {
        
        // File upload configuration
        $config['upload_path'] = '/var/www/html/paceradius/uploads/'; // Ensure this folder is writable
        $config['allowed_types'] = 'jpg|jpeg|png|pdf|doc|docx'; // Allowed file types
        $config['max_size'] = 4096; // Maximum file size in KB (4MB)
        $config['file_name'] = $file_name; // Set the custom file name

        // Initialize upload library with the current config
        $this->upload->initialize($config);

        // Check if the file was uploaded via form
        if (!empty($_FILES[$field_name]['name'])) {
            // Try to upload the file
            if ($this->upload->do_upload($field_name)) {
                // Return the uploaded file name on success
                return $this->upload->data('file_name');
            } else {
                // Handle error and log or return the error message if necessary
                $error = $this->upload->display_errors();
                echo $error; // Optional: Display the error message
    ...
    // Add New User Form Check if user already exists
    function checkUserExist(){

        $username = $this->input->post('userId');

        if($this->users_model->checkUserExist($username) == TRUE)
        {            
            echo "exists";
        ...
        else
        {
            ...
            else
            {
                ...
                $command = "echo User-Name=".$connectedUser->username.",Framed-IP-Address=".$connectedUser->framedipaddress.
                            " | radclient -r l ".$connectedUser->nasipaddress.":3799 disconnect ".$connectedUser->secret;

                $logInfo = 'user:'.$user.'-srvid:'.$srvid.'-editedby:'.$this->session->userdata ( 'name' );
                log_message('info', 'subscribers-list - Edited '.$logInfo);
    ...
    function usersCardsCleanList($seriesID) // Simple Cards Fetch from Series List for Copy Pase
    {
        ...
        else
        {
            ...
            // Build card -> NAS label map so view can show voucher NAS assignment.
    ...
    function generatePrepaidCards($quantity, $cardtype, $cardvalue, $expiration, $prefix, $usergroup, $pinlength,
            ...
            for ($i = 0; $i < $quantity; $i++){
                ...
                if($this->users_model->checkUserExist($newCardNum) == FALSE){
                    ...
                    log_message('info', 'subscribers-list - Created Bulk Prepaid User '.$logInfo);
